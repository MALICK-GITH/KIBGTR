...
try:
    ...
    BACKUP_TEMPLATE_AVAILABLE = True
except ImportError:
...
def admin_login():
    if request.method == 'POST':
        ...
        if user and (check_password(password, user.password) if SECURITY_ENABLED else user.password == password) and user.is_admin:
...
def user_login():
    if request.method == 'POST':
        ...
        user = User.query.filter_by(username=username).first()
        if user and (check_password(password, user.password) if SECURITY_ENABLED else user.password == password):
